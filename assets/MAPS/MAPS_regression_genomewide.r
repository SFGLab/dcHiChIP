...
if (length(args) != 4) {
    ...
    INFDIR = args[1]
    SET = args[2]
    chroms = paste('chr',seq(1,args[3],1),sep='')
    if (args[4] != 'None') {
        FILTER = args[4]
...
for (i in chroms) {
    for (j in c('.and','.xor')) {
        ...
        inf_name = paste(INFDIR,'reg_raw.',i,'.',SET,j,sep='')
        print(inf_name)
...
mm = mm_xor
rm(mm_xor)
outf_name = paste(INFDIR,'reg_raw.',SET,'.xor.MAPS_pospoisson_genomewide',sep = '')
...
mm = mm_and
rm(mm_and)
outf_name = paste(INFDIR,'reg_raw.',SET,'.and.MAPS_pospoisson_genomewide',sep = '')
fit <- vglm(count ~ logl + loggc + logm + logdist + logShortCount, family = pospoisson(), data = mm)
mm$expected = fitted(fit)
mm$p_val = ppois(mm$count, mm$expected, lower.tail = FALSE, log.p = FALSE) / ppois(0, mm$expected, lower.tail = FALSE, log.p = FALSE)
m1 = mm[ mm$p_val > 1/length(mm$p_val),]
